...
from grammar import aminoacid_grammar, allowed_types, nucleotide_grammar
...
class SyntaxRulesTest(unittest.TestCase):
    ...
    def test_aminoacid_cases(self):
        with open('test_data/aminoacid_alleles_fixable.tsv') as ins:
            ins.readline()
            for line in ins:
    ...
    def test_nucleotide_cases(self):
        with open('test_data/nucleotide_alleles_fixable.tsv') as ins:
            ins.readline()
            for line in ins:
                if len(line.strip()) == 0 or line[0] == '#':
                    continue
