from transvar_functions import get_transvar_str_annotation, parse_transvar_string
import unittest
...
class TransvarTest(unittest.TestCase):
    ...
    def test_ganno(self):
        ...
        variant_list = parse_transvar_string(get_transvar_str_annotation('ganno', 'II:g.178497T>A'))
        self.assertEqual(len(variant_list), 3)

        # A wrong chromosome / sequence returns a SequenceRetrievalError
        try:
            get_transvar_str_annotation('ganno', 'blah:g.1000A>T')
        ...
        try:
            get_transvar_str_annotation('ganno', 'I:g.1000A>T')
        ...
        try:
            get_transvar_str_annotation('ganno', 'dummy')
        ...
        try:
            get_transvar_str_annotation('ganno', 'I:blah')
        except InvalidInputError as e:
        ...
        try:
            get_transvar_str_annotation('ganno', 'I:g.1000_1001ins')
        except InvalidInputError as e:
    ...
    def test_panno(self):
        ...
        # A wrong chromosome / sequence returns a SequenceRetrievalError
        try:
            get_transvar_str_annotation('panno', 'blah:p.T566S')
        except Exception as e:
        ...
        try:
            get_transvar_str_annotation('panno', 'SPAPB1A10.09:p.A2F')
        ...
        try:
            get_transvar_str_annotation('panno', 'SPAPB1A10.09:blah')
        except InvalidInputError as e:
            self.assertEqual(e.args[0], "invalid_mutation_string_blah")
        else:
            self.fail("InvalidInputError not raised")
        ...
        get_transvar_str_annotation('panno', 'SPAPB1A10.09:p.10_11ins')

        # A correct annotation that should return two transcripts
        variant_list = parse_transvar_string(get_transvar_str_annotation('panno', 'SPBC1198.04c:p.N3A'))
