...
def check_aminoacid_pos(aa, aa_pos, gene):
    # 1-based index
    ...
    if aa_pos+1 < len(peptide_seq) and peptide_seq[zero_based_pos+1] == aa:
...
def check_sequence_single_aa(groups, gene):
    aa = groups[0]
    aa_pos = int(groups[1])
    return check_aminoacid_pos(aa, aa_pos, gene)
...
def check_sequence_multiple_aa(groups, gene):
    ...
    for i, aa in enumerate(groups[0]):
        results_list.append(check_aminoacid_pos(aa, pos_first_aa + i, gene))
